...
- The user types "review TICKET-ID" (e.g., "review PROJ-123" or "review PROJ-456") to review the branch associated with an issue tracker ticket
...
**If an issue tracker ticket ID is provided** (e.g., "review PROJ-123" or "review PROJ-456"):

A ticket ID matches the pattern: uppercase letters followed by a hyphen and numbers (e.g., `PROJ-123`, `PROJ-456`).
...
1. **Detect build tool**: Check for the following in the worktree root:
   - `pnpm-lock.yaml` → use pnpm
   - `package-lock.json` → use npm
   - `gradlew` → use Gradle wrapper
2. **Install dependencies / verify build**:
   ...
   # Gradle project
   cd <worktree-path> && ./gradlew build
   ```
...
**If a branch name is provided** (e.g., "review PROJ-738-show-manage-admins-button"):
1. Fetch latest from origin: `git fetch origin`
...
If an issue tracker ticket ID was detected, fetch ticket details:
...
If no MCP tool is connected, ask the user to provide the ticket details or skip this step.
...
- Verify code review is within the acceptance criteria of the associated ticket (if available)
