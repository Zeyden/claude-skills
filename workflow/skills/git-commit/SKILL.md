...
Generate Conventional Commits messages that tell a complete story for future code archeology, with optional issue tracker context.
...
**Do not add Co-Author to the git commit message.**
...
- If neither MCP tool is connected, skip this step.
...
Based on the diff and ticket context (if available), generate 3-4 plausible options for why the change was made. Present these as multiple choice options using AskUserQuestion.
...
[Incorporate the user's explanation and ticket context]
...
- **What problem** does it solve? (from ticket context and user input)
- **How** does it relate to the larger feature/project? (ticket context)

## Examples

### Example 1: Feature with Issue Tracker Context
...
- **Reference issue tracker tickets (Jira or Linear)** when found in branch name
